...
Follow `rules/roe.md` at all times.
...
- The exact in-scope assets: domains, subdomains, wildcards, mobile apps, IP ranges, API hosts.
...
- **Wildcard scope** — broad or partially unknown attack surface (e.g. `*.example.com`, multiple undocumented subdomains/apps). This routes `alr-recon` into heavy subdomain/asset enumeration before any per-app deep dive.
- **Main-domain scope** — a single, specific host or application. This routes `alr-recon` straight into authenticated, manual exploration and JS mining of that one app.
